...
Public Class Recep_Mue_PENDIENTES_2
    ...
    <Services.WebMethod()>
    Public Shared Function Form_Table(ByVal ID_USU As Integer) As List(Of E_IRIS_WEBF_BUSCA_ETIQUETAS_RECEP_USUARIOS4____4)
        ...
        data_paciente = NN.IRIS_WEBF_CMVM_BUSCA_ETIQUETAS_RECEP_USUARIOS4____4_CON_PENDIENTES_666(ID_USER)           '1 = ID USUARIO
    ...
    Public Shared Function Llenar_DataTable_Laser(ByVal NUM_ATE As String, ByVal ID_USU As Integer) As Object
        ...
        If (NUM_ATE.Length) < 10 Then
            Return (New N_IRIS_WEBF_BUSCA_ANTECENDES_PACIENTE_RECEP).IRIS_WEBF_BUSCA_ANTECENDES_PACIENTE_RECEP(NUM_ATE)
        End If
        Dim objSession As HttpSessionState = HttpContext.Current.Session
        Dim ID_USER As Integer = ID_USU
        ...
        Return D_IRIS_WEBF_UPDATE_ESTADO_RECEPCIONADO_RECEPCION.IRIS_WEB_GRABA_RECEPCION_ETIQUETA_Y_UPDATE_ATE_RESULTADO_LASER(ATE_NUM, CB, ID_USER)
    ...
    Public Shared Function Guardar_de_una(ByVal NUM_ATE As String, ByVal ID_USU As Integer) As String
        ...
        Dim data_paciente_deuna As List(Of E_IRIS_WEBF_BUSCA_ETIQUETA_POR_FOLIO_RECEPCION_DE_ETIQUETAS)
        Dim NN_deuna As N_IRIS_WEBF_BUSCA_ETIQUETA_POR_FOLIO_RECEPCION_DE_ETIQUETAS = New N_IRIS_WEBF_BUSCA_ETIQUETA_POR_FOLIO_RECEPCION_DE_ETIQUETAS

        Dim hola As String = ""

        data_paciente_deuna = NN_deuna.IRIS_WEBF_BUSCA_ETIQUETA_POR_FOLIO_RECEPCION_DE_ETIQUETAS(NUM_ATE)

        If data_paciente_deuna.Count > 0 Then
            For ii = 0 To data_paciente_deuna.Count - 1

                data_paciente2 = NN2.IRIS_WEBF_BUSCA_ETIQUETA_BARRA_PACIENTE_RECEP(NUM_ATE, data_paciente_deuna(ii).CB_DESC)
                If data_paciente2.Count > 0 Then
                    For i = 0 To data_paciente2.Count - 1
                        update_paciente3 = NN3.IRIS_WEBF_UPDATE_ESTADO_RECEPCIONADO_RECEPCION(data_paciente2(i).ID_ATE_RES, ID_USER)
                    Next i

                    data_paciente4 = NN4.IRIS_WEBF_BUSCA_ETIQUETA_GUARDADA_RECEPCION_ETIQUETAS(data_paciente2(0).ID_ATENCION, data_paciente2(0).ATE_NUM, data_paciente2(0).CB_COD, ID_USER)

                    If data_paciente4.Count > 0 Then
                        update_paciente5 = 0
                    Else
                        update_paciente5 = NN5.IRIS_WEBF_GRABA_ETIQUETA_RECEPCION_ETIQUETA(data_paciente2(0).ID_ATENCION, data_paciente2(0).ATE_NUM, data_paciente2(0).CB_COD, ID_USER)
                    End If
                End If
    ...
    Public Shared Function Llenar_DataTable_Marcar(ByVal NUM_ATE As String, ByVal MUESTRA() As Object, ByVal ID_USU As Integer) As String
        ...
        For ii = 0 To (MUESTRA.GetUpperBound(0))
            ...
            If data_paciente.Count > 0 Then
                'esta wea es un listado de los resultados (iris_ate_resultado) de la atención no los pacientes por las rechuchas
                data_paciente2 = NN2.IRIS_WEBF_BUSCA_ETIQUETA_BARRA_PACIENTE_RECEP(NUM_ATE, CB)

                If data_paciente2.Count > 0 Then
                    For i = 0 To data_paciente2.Count - 1
                        'update cada uno de ate resultado que se contiene en la etiqueta (CB) por la puta se podía haber hecho en el pa de arriba
                        update_paciente3 = NN3.IRIS_WEBF_UPDATE_ESTADO_RECEPCIONADO_RECEPCION(data_paciente2(i).ID_ATE_RES, ID_USER) '1 = ID USUARIO
                    Next i
    ...
    '        Dim save = N_IRIS_WEBF_UPDATE_ESTADO_RECEPCIONADO_RECEPCION.IRIS_WEB_GRABA_RECEPCION_ETIQUETA_Y_UPDATE_ATE_RESULTADO(ID_ATENCION, NUM_ATE, cb, ID_USER)
    '    Next
    '    Return "tables"
    'End Function

    <Services.WebMethod()>
    Public Shared Function Llenar_DataTable_Marcar_Nuevo(ByVal NUM_ATE As String, ByVal MUESTRA() As Object, ByVal ID_USU As Integer) As String
        ...
        For ii = 0 To MUESTRA.GetUpperBound(0)
            ...
            data_paciente6 = NN6.IRIS_WEBF_BUSCA_ETIQUETAS_RECEP_USUARIOS4____4(ID_USER)

            If data_paciente6.Count > 0 Then
                For i = 0 To data_paciente6.Count - 1
                    If String.IsNullOrEmpty(data_paciente6(i).RECEP_ETI_CURVA) OrElse String.IsNullOrEmpty(data_paciente6(i).RECEP_ETI_NUM_ATE) Then
                        Return "recepcionada"
                    End If
                Next i
    ...
    Public Shared Function Llenar_DataTable3(ByVal N_ATE As String, ByVal ID_USU As Integer) As String
        ...
        'data_paciente = NN.IRIS_WEBF_BUSCA_ETIQUETA_POR_FOLIO_RECEPCION_DE_ETIQUETAS(N_ATE)    '<------ *************************************************************************** PENDIENTE
        data_paciente = NN.IRIS_WEBF_CMVM_BUSCA_ETIQUETA_POR_FOLIO_RECEPCION_DE_ETIQUETAS_4(N_ATE)
        ...
        If selected.Length > 0 Then
            For i = 0 To selected.Length - 1
                If selected(i).length > 2 Then
                    ...
                    data_elimina = (New N_IRIS_WEBF_ELIMINA_RECEPCION_ETIQUETA_2).IRIS_WEBF_ELIMINA_RECEPCION_ETIQUETA_2(elemento(1), elemento(0))

                    'DataExamenDet = NN_ExamenDet.IRIS_WEBF_CMVM_UPDATE_ESTADO_EXAMEN_PENDIENTES_CON_OMI(elemento(4), elemento(5))

                    Data_Register = D_Exa_Esp_V.IRIS_WEBF_CMVM_GRABA_REGISTRO_PENDIENTES_TIPO(ID_USER, elemento(0), elemento(1), elemento(2), elemento(5), elemento(6), Date.Now, USU_ID_PROC, 1)
    ...
    '                                             ByVal MULTIPLICADIIIIIS As String) As String
    '    'Declaraciones del Serializador
    '    Dim Serializer As New JavaScriptSerializer
    '    Dim str_Builder As New StringBuilder
    '    Dim datas As String = ""
    ...
    '        If (MULTIPLICADIIIIIS <> "") Then
    '            Dim regex As New Regex("[0-9]+", RegexOptions.ECMAScript And RegexOptions.IgnoreCase)
    '            Dim match9000 As MatchCollection
    '            Dim list_Num As New List(Of String)

    '            match9000 = regex.Matches(MULTIPLICADIIIIIS)
    '            For y = 0 To (match9000.Count - 1)
    '                If (match9000(y).Success = True) Then
    ...
    Public Shared Function Llenar_Modal_Cargar_Doble_click(ByVal NUM_ATE As String) As String
        ...
        data_paciente = NN.IRIS_WEBF_CMVM_BUSCA_ETIQUETA_POR_FOLIO_RECEPCION_DE_ETIQUETAS_4(NUM_ATE)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim objSession As HttpSessionState = HttpContext.Current.Session
        If Request.Cookies.[Get]("P_ADMIN") Is Nothing Then

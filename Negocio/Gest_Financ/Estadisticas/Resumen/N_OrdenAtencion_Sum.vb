...
Imports System.Web
Public Class N_OrdenAtencion_Sum
    Dim DD_Gen_Activos As D_Gen_Activos
    Dim DD_Prev As D_OrdenAtencion_Sum
    Sub New()
        DD_Gen_Activos = New D_Gen_Activos
        DD_Prev = New D_OrdenAtencion_Sum
    End Sub
    Function IRIS_BUSCA_WEBF_LIS_ADM_RESU_ORDEN(ByVal ID_PRE As Long, ByVal Date_01 As Date, ByVal Date_02 As Date) As List(Of E_IRIS_WEBF_BUSCA_LIS_ADM_RESU_ORDEN)
        Return DD_Prev.IRIS_BUSCA_WEBF_LIS_ADM_RESU_ORDEN(ID_PRE, Date_01, Date_02)
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal PREVE As Long, ByVal DATE1 As String, ByVal DATE2 As String) As String
        ...
        Dim NN_Aten As New N_OrdenAtencion_Sum
        Dim Data_Aten As New List(Of E_IRIS_WEBF_BUSCA_LIS_ADM_RESU_ORDEN)
        Dim Mx_Data(6, 0) As Object
        'Obtener parámetros para la consulta
        Dim Previs As Long = PREVE
        'Realizar Consulta
        Data_Aten = NN_Aten.IRIS_BUSCA_WEBF_LIS_ADM_RESU_ORDEN(Previs, DATE1, DATE2)
        ...
        For y = 0 To (Data_Aten.Count - 1)
            ...
            End If
            Mx_Data(0, y) = Data_Aten(y).ORD_DESC
        ...
        Else
            sl.SetCellValue("B3", "Prioridad TM: " & Data_Aten(0).ORD_DESC)
        End If
        ...
        Else
            Dim Ruta_save_local As String = System.Web.HttpContext.Current.Server.MapPath("~/")
            Dim Relative_Path As String = "IRISPDFDERIVADOS\" & Data_Aten(0).ORD_DESC & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
            sl.SaveAs(Ruta_save_local & Relative_Path) 'Abrir el Archivo
            'Devolver la url del archivo generado
            Return MAIN_URL & "/" & Replace(Relative_Path, "\", "/")
        End If
    End Function
    Private Function IRIS_WEBF_BUSCA_LIS_ADM_RESU_PREVISION(Previs As Long, Date01 As Date, Date02 As Date) As List(Of E_IRIS_WEBF_BUSCA_LIS_ADM_RESU_ORDEN)

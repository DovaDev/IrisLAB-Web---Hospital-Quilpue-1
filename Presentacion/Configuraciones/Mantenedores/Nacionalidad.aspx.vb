...
Public Class Nacionalidad
    ...
    End Function
    <Services.WebMethod()>
    Public Shared Function IRIS_WEBF_UPDATE_NACIONALIDAD(ByVal ID_NAC As Integer, ByVal NAC_COD As String, ByVal NAC_DES As String, ByVal ID_ESTADO As Integer) As String
        'Declaraciones del Serializador
        Dim Serializer As New JavaScriptSerializer
        Dim str_Builder As New StringBuilder
        Dim datas As String = ""
        ...
        Dim NN As N_IRIS_WEBF_UPDATE_NACIONALIDAD = New N_IRIS_WEBF_UPDATE_NACIONALIDAD
        numerin = NN.IRIS_WEBF_UPDATE_NACIONALIDAD(ID_NAC, NAC_COD, NAC_DES, ID_ESTADO)
        If (numerin > 0) Then
            'Serializar con JSON
            Serializer.MaxJsonLength = 999999999
    ...
    End Function
    <Services.WebMethod()>
    Public Shared Function IRIS_WEBF_GRABA_NACIONALIDAD(ByVal NAC_COD As String, ByVal NAC_DES As String, ByVal ID_ESTADO As Integer) As String
        'Declaraciones del Serializador
        Dim Serializer As New JavaScriptSerializer
        Dim str_Builder As New StringBuilder
        Dim datas As String = ""
        'Declaraciones internas
        Dim numerin As Integer = 0
        Dim NN As N_IRIS_WEBF_GRABA_NACIONALIDAD = New N_IRIS_WEBF_GRABA_NACIONALIDAD
        numerin = NN.IRIS_WEBF_GRABA_NACIONALIDAD(NAC_COD, NAC_DES, ID_ESTADO)
    ...
    Public Shared Function Excel(ByVal DOMAIN_URL As String) As String
        ...
        Dim NN As N_IRIS_WEBF_BUSCA_NACIONALIDAD = New N_IRIS_WEBF_BUSCA_NACIONALIDAD
        data_paciente = NN.IRIS_WEBF_BUSCA_NACIONALIDAD()
        Dim titulo As String = "Nacionalidad"
        Dim Mx(3, 0) As Object
        For y = 0 To (data_paciente.Count - 1)
            If (y > 0) Then
                ReDim Preserve Mx(3, y)
            End If
            Mx(0, y) = data_paciente(y).ID_NACIONALIDAD
            Mx(1, y) = data_paciente(y).NAC_COD
            Mx(2, y) = data_paciente(y).NAC_DESC

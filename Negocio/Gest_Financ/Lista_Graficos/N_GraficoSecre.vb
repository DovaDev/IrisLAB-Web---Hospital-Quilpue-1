...
Imports Datos
Imports Entidades
Public Class N_GraficoSecre
    Dim DD_Data As D_GraficoSecre
    Sub New()
        DD_Data = New D_GraficoSecre
    End Sub
    Function IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_USUARIO(ByVal PAC As Long, ByVal Año As Long, ByVal Mes As Long) As List(Of E_IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_USUARIO)
        Return DD_Data.IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_USUARIO(PAC, Año, Mes)
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal Dll As Long, ByVal Año As String) As String
        ...
        Dim NN_Prev As New N_GraficoSecre
        ...
        Dim data_Prev As New List(Of E_IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_USUARIO)
        ...
        For i = 0 To Data_Proce.Count - 1
            If Data_Proce(i).ID_USUARIO = Dll Then
                Prev_Desc = Data_Proce(i).USU_NOMBRE & " " & Data_Proce(i).USU_APELLIDO
            End If
        Next i
        ...
        For dd = 1 To 12
            ...
            If Dll <> 0 Then
                data_Prev = NN_Prev.IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_USUARIO(Dll, Año, dd)
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Atenciones Totales Por Previsión")
        ''titulo de la tabla
        sl.SetCellValue("B2", "Atenciones Totales (Listado)")
        If (Dll = "0") Then
            sl.SetCellValue("B3", "Previsión: Todas")
        Else
            sl.SetCellValue("B3", "Previsión: " & Prev_Desc)
        End If
        ...
        If (Dll = 0) Then
            ...
            Dim Relative_Path As String = "IRISPDFDERIVADOS\Todos" & Año & "Todas" & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
            sl.SaveAs(Ruta_save_local & Relative_Path) 'Abrir el Archivo
            'Devolver la url del archivo generado
            Return MAIN_URL & "/" & Replace(Relative_Path, "\", "/")
        Else
            Dim Ruta_save_local As String = System.Web.HttpContext.Current.Server.MapPath("~/")
            Dim Relative_Path As String = "IRISPDFDERIVADOS\" & Año & Prev_Desc & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
            sl.SaveAs(Ruta_save_local & Relative_Path) 'Abrir el Archivo
            'Devolver la url del archivo generado
            Return MAIN_URL & "/" & Replace(Relative_Path, "\", "/")
        End If

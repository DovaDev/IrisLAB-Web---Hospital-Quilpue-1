...
Public Class N_REP_EXA_SECR
    ...
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal ID_CODIGO_FONASA As Long, ByVal DATE1 As String, ByVal DATE2 As String) As String
        ...
        Dim NN_Exam As New N_REP_EXA_SECR
        Dim Data_Exam As New List(Of E_IRIS_WEBF_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION)
        ...
        'Realizar Consulta
        Data_Exam = NN_Exam.IRIS_WEBF_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION(ID_CODIGO_FONASAA, Date01, Date02)
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Cantidad de Exámenes por Sección")
        'titulo de la tabla
        sl.SetCellValue("B2", "Cantidad de Exámenes por Sección")
        If (ID_CODIGO_FONASAA = 0) Then
            sl.SetCellValue("B3", "Seccion: Todas")
        Else
            sl.SetCellValue("B3", "Seccion: " & Data_Exam(0).SECC_DESC)
        End If
        ...
        Else
            Dim Ruta_save_local As String = System.Web.HttpContext.Current.Server.MapPath("~/")
            Dim Relative_Path As String = "IRISPDFDERIVADOS\" & Data_Exam(0).SECC_DESC & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
            sl.SaveAs(Ruta_save_local & Relative_Path) 'Abrir el Archivo
            'Devolver la url del archivo generado
            Return MAIN_URL & "/" & Replace(Relative_Path, "\", "/")
        End If

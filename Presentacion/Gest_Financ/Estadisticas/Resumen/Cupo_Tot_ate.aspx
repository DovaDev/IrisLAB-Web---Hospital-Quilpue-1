<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Master.Master" CodeBehind="Cupo_Tot_ate.aspx.vb" Inherits="Presentacion.Cupo_Tot_ate" %>
...
<%--    <script src="/js/HighCharts.js"></script>
    ...
    <script>
        ...
        function Fill_AJAX_Ddl() {
            ...
            if (procee == 0) {
                $("<option>",
                   ...
                   ).text("Todos").appendTo("#Procedencia");
                Mx_Ddl.forEach(aaa => {
                    $("<option>",
                        {
        ...
        function Ajax_DataTable() {
            ...
            $.ajax({
                "type": "POST",
                "url": "Cupo_Tot_ate.aspx/Llenar_DataTable",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
        ...
        function Ajax_Excel() {
            ...
            $.ajax({
                "type": "POST",
                "url": "Cupo_Tot_ate.aspx/Gen_Excel",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
            ...
            for (i = 0; i < Mx_Dtt.length; i++) {
                $("#DataTable tbody").append(
                    $("<tr>").append(
                        ...
                        $("<td>", { "align": "center" }).text(Mx_Dtt[i].TOTAL_AGEND_CUPO_NORMAL),
                        $("<td>", { "align": "center" }).text(Mx_Dtt[i].TOTAL_AGEND_PRIORITARIO),
                        $("<td>", { "align": "center" }).text(Mx_Dtt[i].TOTAL_AGEND_ESPONTANEO),
                        $("<td>", { "align": "center" }).text(Mx_Dtt[i].TOTAL_AGEND_PAP),
                           $("<td>", { "align": "center" }).text(Mx_Dtt[i].TOTAL_SOBRE_CUPO),
                        $("<td>", { "align": "center" }).text(Mx_Dtt[i].TOTAL_AGEND_PAP + Mx_Dtt[i].TOTAL_AGEND_ESPONTANEO + Mx_Dtt[i].TOTAL_AGEND_PRIORITARIO + Mx_Dtt[i].TOTAL_AGEND_CUPO_NORMAL + Mx_Dtt[i].TOTAL_SOBRE_CUPO)

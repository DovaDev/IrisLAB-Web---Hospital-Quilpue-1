...
Imports Datos
Imports Entidades
Public Class N_GraficoAteOrd
    Dim DD_Data As D_GraficoAteOrd
    Dim DD_Data_Todo As D_GraficoTM_Todo
    Sub New()
        DD_Data = New D_GraficoAteOrd
        DD_Data_Todo = New D_GraficoTM_Todo
    End Sub
    Function IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_ORDEN_ATE(ByVal ID_PAC As Long, ByVal Año As Long, ByVal Mes As Long) As List(Of E_IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_ORDEN_ATE)
        Return DD_Data.IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_ORDEN_ATE(ID_PAC, Año, Mes)
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal Dll As Long, ByVal Año As String) As String
        ...
        For i = 0 To Data_Proce.Count - 1
            If Data_Proce(i).ID_ORDEN = Dll Then
                TP_Desc = Data_Proce(i).ORD_DESC
            End If
        Next i
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Cantidad Anuales por Orden de Atencion")
        ''titulo de la tabla
        sl.SetCellValue("B2", "Cantidad Anuales por Orden de Atencion")
        If (Dll = "0") Then
            sl.SetCellValue("B3", "Orden de Atención: Todas")
        Else
            sl.SetCellValue("B3", "Orden de Atención: " & TP_Desc)
        End If

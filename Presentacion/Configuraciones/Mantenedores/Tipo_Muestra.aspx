<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Master.Master" CodeBehind="Tipo_Muestra.aspx.vb" Inherits="Presentacion.Tipo_Muestra" %>
<asp:Content ID="Content1" ContentPlaceHolderID="Cph_Head" runat="server">
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#Btn_Modificar").click(function () {
                if (($("#Ddl_Mantenedor").val() == 0) && ($("#Ddl_CB").val() == 0) && ($("#Ddl_RECEP").val() == 0)) {
            ...
            $("#Btn_Guardar").click(function () {
                if (($("#txtCod").val() == "") || ($("#txtDesc").val() == "") || ($("#Ddl_Mantenedor").val() == 0) || ($("#Ddl_CB").val() == 0) || ($("#Ddl_RECEP").val() == 0)) {
                    $("#mError_AAH h4").text("Error");
                    $("#mError_AAH button").attr("class", "btn btn-danger");
    ...
    <script>
        function Ajax_Codiguin(COD, DESC, CB, RECEP, ESTADO, ID) {
            $("#txtCod").val(COD);
            $("#txtDesc").val(DESC);
            $("#Ddl_CB").val(CB);
            $("#Ddl_RECEP").val(RECEP);
        ...
        function Ajax_Ddl_Mantenedor() {
            ...
            $.ajax({
                "type": "POST",
                "url": "Tipo_Muestra.aspx/IRIS_WEBF_BUSCA_ESTADO_MANTENEDOR",
                //"data": Data_Par,
        ...
        function Ajax_Ddl_CB() {
            modal_show();

            $(".block_wait").fadeIn(500);
            $.ajax({
                "type": "POST",
                "url": "Tipo_Muestra.aspx/IRIS_WEBF_BUSCA_CB_MANTENEDOR",
                //"data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
                    var json_receiver = response.d;
                    if (json_receiver != "null") {
                        Mx_Dtt_CB = JSON.parse(json_receiver);
                        Fill_Ddl_CB();
                        Hide_Modal();
        ...
        function Ajax_Ddl_RECEP() {
            modal_show();

            $(".block_wait").fadeIn(500);
            $.ajax({
                "type": "POST",
                "url": "Tipo_Muestra.aspx/IRIS_WEBF_BUSCA_RECEP_MANTENEDOR",
                //"data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
                    var json_receiver = response.d;
                    if (json_receiver != "null") {
                        Mx_Dtt_RECEP = JSON.parse(json_receiver);
                        Fill_Ddl_RECEP();
                        Hide_Modal();
        ...
        function Ajax_Tabla() {
            ...
            $.ajax({
                "type": "POST",
                "url": "Tipo_Muestra.aspx/IRIS_WEBF_BUSCA_TP_MUESTRA",
                //"data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
        ...
        function Ajax_Update() {
            ...
            var Data_Par = JSON.stringify({
                ...
                "RE_DES": $("#txtDesc").val(),
                "ID_CB": $("#Ddl_CB").val(),
                "ID_RECEP": $("#Ddl_RECEP").val(),
                "ID_ESTADO": $("#Ddl_Mantenedor").val()
            });
            $(".block_wait").fadeIn(500);
            $.ajax({
                "type": "POST",
                "url": "Tipo_Muestra.aspx/IRIS_WEBF_UPDATE_TP_MUESTRA",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
        ...
        function Ajax_Guardar() {
            ...
            $.ajax({
                "type": "POST",
                "url": "Tipo_Muestra.aspx/IRIS_WEBF_GRABA_TP_MUESTRA",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
    ...
    <script>
        ...
        //Llenar DropDownList Tipo de Atención
        function Fill_Ddl_CB() {
            $("#Ddl_CB").empty();
            $("<option>", { "value": 0 }).text("Seleccionar").appendTo("#Ddl_CB");
            for (y = 0; y < Mx_Dtt_CB.length; ++y) {
                $("<option>", {
                    "value": Mx_Dtt_CB[y].ID_CB
                }).text(Mx_Dtt_CB[y].CB_DES).appendTo("#Ddl_CB");
            }
        };
        //Llenar DropDownList Tipo de Atención
        function Fill_Ddl_RECEP() {
            $("#Ddl_RECEP").empty();
            $("<option>", { "value": 0 }).text("Seleccionar").appendTo("#Ddl_RECEP");
            for (y = 0; y < Mx_Dtt_RECEP.length; ++y) {
                $("<option>", {
                    "value": Mx_Dtt_RECEP[y].ID_GMUE
                }).text(Mx_Dtt_RECEP[y].GMUE_DES).appendTo("#Ddl_RECEP");
            }
        };
    </script>
    <script>
        ...
        function Fill_DataTable() {
            ...
            for (i = 0; i < Mx_Dtt.length; i++) {
                $("#DataTable tbody").append(
                    $("<tr>", {
                        "onclick": `Ajax_Codiguin("` + Mx_Dtt[i].RE_COD + `","` + Mx_Dtt[i].RE_DES + `","` + Mx_Dtt[i].ID_CB + `","` + Mx_Dtt[i].ID_RECEP + `","` + Mx_Dtt[i].ID_ESTADO + `","` + Mx_Dtt[i].ID_RE + `")`,
                        "class": "manito"
                    }).append(
                        $("<td>", { "align": "left" }, { "class": "textoReducido" }).text(i + 1),
                        $("<td>", { "align": "left" }, { "class": "textoReducido" }).text(Mx_Dtt[i].RE_COD),
                        $("<td>", { "align": "left" }, { "class": "textoReducido" }).text(Mx_Dtt[i].RE_DES),
...
<asp:Content ID="Content2" ContentPlaceHolderID="Cph_Body" runat="server">
    ...
    <div class="row">
        <div class="col-lg">
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    <label for="Ddl_CB">Codigo Barra:</label>
                    <select id="Ddl_CB" class="form-control textoReducido mayus"></select>
                </div>
                <div class="col-md-4">
                    <label for="Ddl_RECEP">Recipiente:</label>
                    <select id="Ddl_RECEP" class="form-control textoReducido mayus"></select>
                </div>
            ...
            <div class="row mb-3" id="Id_Conte">
                <div class="col-md-12" id="Paciente">
                    <h5 style="text-align: center; padding: 5px; font-size: 15px;"><i class="fa fa-list"></i>Lista de Indicaciones Médicas</h5>

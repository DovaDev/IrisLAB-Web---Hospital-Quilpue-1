...
Imports Datos
Imports Entidades
Public Class N_Ana_PAC
    Dim DD_Data As D_Ana_PAC
    Dim DD_Data_Todo As D_GraficoTM_Todo
    Sub New()
        DD_Data = New D_Ana_PAC
    ...
    Function IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_TP_PACI(ByVal ID_PAC As Long, ByVal Año As Long, ByVal Mes As Long) As List(Of E_IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_TP_PACI)
        Return DD_Data.IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_TP_PACI(ID_PAC, Año, Mes)
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal Dll As Long, ByVal Año As String) As String
        ...
        Dim NN_MM As New N_Ana_PAC
        Dim Data_Med As New List(Of E_IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_POR_LUGAR_TM1_TODOS)
        Dim nnnnn As New List(Of E_IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_TP_PACI)
        Dim date_json_rial As New List(Of E_GraficoVisor_Json)
        'Declaraciones Internas
        Dim NN_Activos As New N_Gen_Activos
        Dim Data_Medi_Activo As New List(Of E_IRIS_WEBF_BUSCA_TP_ATENCION_ACTIVO)
        Dim TP_Desc As String = ""
        Dim mm As Integer
        mm = Format(Date.Now, "MM")
        'Consultar por previsiones activas
        Data_Medi_Activo = NN_Activos.IRIS_WEBF_BUSCA_TP_ATENCION_ACTIVO
        For i = 0 To Data_Medi_Activo.Count - 1
            If Data_Medi_Activo(i).ID_TP_ATENCION = Dll Then
                TP_Desc = Data_Medi_Activo(i).TP_ATE_DESC
            End If
        Next i
        ...
        Else
            For dd = 1 To 12
                Dim Item As New E_GraficoVisor_Json
                nnnnn = NN_MM.IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_TP_PACI(Dll, Año, dd)
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Cantidad Anuales por Paciente")
        ''titulo de la tabla
        sl.SetCellValue("B2", "Cantidad Anuales por Paciente")
        If (Dll = "0") Then
            sl.SetCellValue("B3", "Paciente: Todos")
        Else
            sl.SetCellValue("B3", "Paciente: " & TP_Desc)
        End If

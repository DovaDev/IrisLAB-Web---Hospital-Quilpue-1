...
Imports Datos
Imports Entidades
Public Class N_Ana_MED
    Dim DD_Data As D_Ana_MED
    Dim DD_Data_Todo As D_GraficoTM_Todo
    Sub New()
        DD_Data = New D_Ana_MED
        DD_Data_Todo = New D_GraficoTM_Todo
    End Sub
    Function IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_MEDICOS(ByVal ID_PAC As Long, ByVal Año As Long, ByVal Mes As Long) As List(Of E_IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_MEDICOS)
        Return DD_Data.IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_MEDICOS(ID_PAC, Año, Mes)
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal Dll As Long, ByVal Año As String) As String
        ...
        Dim NN_MM As New N_Ana_MED
        Dim Data_Med As New List(Of E_IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_POR_LUGAR_TM1_TODOS)
        Dim nnnnn As New List(Of E_IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_MEDICOS)
        ...
        For i = 0 To Data_Medi_Activo.Count - 1
            If Data_Medi_Activo(i).ID_DOCTOR = Dll Then
                TP_Desc = Data_Medi_Activo(i).DOC_NOMBRE & " " & Data_Medi_Activo(i).DOC_APELLIDO
            End If
        Next i
        ...
        Else
            For dd = 1 To 12
                Dim Item As New E_GraficoVisor_Json
                nnnnn = NN_MM.IRIS_WEBF_BUSCA_CANT_EXAMENES_ATE_MEDICOS(Dll, Año, dd)
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Cantidad Anuales por Médico")
        ''titulo de la tabla
        sl.SetCellValue("B2", "Cantidad Anuales por Médico")
        If (Dll = "0") Then
            sl.SetCellValue("B3", "Médico: Todos")
        Else
            sl.SetCellValue("B3", "Médico: " & TP_Desc)
        End If

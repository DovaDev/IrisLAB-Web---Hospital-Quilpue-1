...
Public Class Lis_Deriv_Exa
    ...
    End Function
    <Services.WebMethod()>
    Public Shared Function Llenar_Ddl_Examen() As List(Of E_IRIS_WEBF_BUSCA_EST_CODIGO_FONASA_ACTIVOS_DERIVADOS)
        'Declaraciones internas
        Dim NN_Exam As New N_IRIS_WEBF_BUSCA_EST_CODIGO_FONASA_ACTIVOS_DERIVADOS
        Dim Data_Exam As New List(Of E_IRIS_WEBF_BUSCA_EST_CODIGO_FONASA_ACTIVOS_DERIVADOS)
        Data_Exam = NN_Exam.IRIS_WEBF_BUSCA_EST_CODIGO_FONASA_ACTIVOS_DERIVADOS()
        If (Data_Exam.Count > 0) Then
            Return Data_Exam
        Else
            Return Nothing
    ...
    <Services.WebMethod()>
    Public Shared Function Call_DataTable(ByVal DATE_str01 As String, ByVal DATE_str02 As String, ByVal ID_PROC As Long, ByVal ID_CF As Long) As List(Of E_IRIS_WEBF_BUSCA_EST_LUGAR_TM_TODOS_POR_ID_TM_POR_DERIVADO)
        'Declaraciones del Serializador
        Dim Serializer As New JavaScriptSerializer
        Dim str_Builder As New StringBuilder

        Dim NN_Table As New N_IRIS_WEBF_BUSCA_EST_LUGAR_TM_TODOS_POR_ID_TM_POR_DERIVADO
        Dim List_Obj As New List(Of E_IRIS_WEBF_BUSCA_EST_LUGAR_TM_TODOS_POR_ID_TM_POR_DERIVADO)
        Dim N_ECrypt As New N_Encrypt
        List_Obj = NN_Table.IRIS_WEBF_BUSCA_EST_LUGAR_TM_TODOS_POR_ID_TM_POR_DERIVADO(DATE_str01, DATE_str02, ID_PROC, ID_CF)
    ...
    <Services.WebMethod()>
    Public Shared Function Gen_Excel(ByVal DOMAIN_URL As String, ByVal DATE_str01 As String, ByVal DATE_str02 As String, ByVal ID_PROC As Long, ByVal ID_CF As Long) As String
        'Declaraciones Internas
        Dim NN_Activos As New N_Gen_Activos
        Dim List_Activo_01 As New List(Of E_IRIS_WEBF_BUSCA_PROCEDENCIA_ACTIVO)
        Dim List_Activo_02 As New List(Of E_IRIS_WEBF_BUSCA_EST_CODIGO_FONASA_ACTIVOS)

        List_Activo_01 = NN_Activos.IRIS_WEBF_BUSCA_PROCEDENCIA_ACTIVO
        List_Activo_02 = NN_Activos.IRIS_WEBF_BUSCA_EST_CODIGO_FONASA_ACTIVOS

        Dim Select_Proce As String = "Todos"
        For y = 0 To (List_Activo_01.Count - 1)
            If (List_Activo_01(y).ID_PROCEDENCIA = ID_PROC) Then
        ...
        For y = 0 To (List_Activo_02.Count - 1)
            If (List_Activo_02(y).ID_CODIGO_FONASA = ID_CF) Then
                Select_Secc = List_Activo_02(y).CF_DESC
                Exit For
            End If
        Next y


        Dim DD_Data = New D_IRIS_WEBF_BUSCA_EST_LUGAR_TM_TODOS_POR_ID_TM_POR_DERIVADO

        'Consulta Principal
        Dim Data_OUT As New List(Of E_IRIS_WEBF_BUSCA_EST_LUGAR_TM_TODOS_POR_ID_TM_POR_DERIVADO)
        Data_OUT = DD_Data.IRIS_WEBF_BUSCA_EST_LUGAR_TM_TODOS_POR_ID_TM_POR_DERIVADO(DATE_str01, DATE_str02, ID_PROC, ID_CF)
        ...
        For y = 0 To (Data_OUT.Count - 1)
            ...
            Mx_Data(9, y) = Data_OUT(y).SEXO_DESC
            Mx_Data(10, y) = Data_OUT(y).ATE_NUM_INTERNO
            Mx_Data(11, y) = Data_OUT(y).PROGRA_DESC
            Mx_Data(12, y) = Data_OUT(y).SECTOR_DESC
        ...
        Xls.SetCellValue(4, 1, "Examen: " & Select_Secc)
        ...
        Xls.SetCellValue(tablePosRow, tablePosCol_now, "Sexo") : tablePosCol_now += 1
        ...
        Dim Relative_Path As String = "IRISPDFDERIVADOS\Lis_Deriv_Exa" & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"

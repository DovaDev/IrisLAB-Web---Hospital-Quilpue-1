...
/**
 * Busca los sectores y llena el o los select que se le pasan.
 *
 * @async
 * @param {Object} [options] - Optional settings - si no se le pasa nada, retorna un listado con todos los sectores.
 ...
 * @returns {Promise<{ID_SECTOR: number, SECTOR_COD: string, SECTOR_DESC: string, ID_ESTADO: number}[]>}
 */
const fillSectores = async ({ idSelect = "", placeholder = false, placeholderText = "Seleccione", defecto = 0 } = {}) => {
    ...
    const response = await fetcher('/Index.aspx/buscarSectores', { afterResOk: resOk, method: "post" });
    return Promise.resolve(response);
};
export default fillSectores;
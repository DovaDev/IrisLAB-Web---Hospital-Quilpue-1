...
Public Class Excel_TP_Real_ADM
    ...
    Public Shared Function Llenar_DataTable(ByVal DESDE As String, ByVal HASTA As String, ByVal ID_TM As String) As String
        ...
        data = NN.IRIS_WEBF_BUSCA_EST_LUGAR_TM_EXCEL(DESDE, HASTA, ID_TM)
    ...
    Public Shared Function Excel(ByVal DOMAIN_URL As String, ByVal DESDE As String, ByVal HASTA As String, ByVal ID_TM As String) As String
        ...
        If (data.Count > 0) Then
            ...
            For y = 0 To (data.Count - 1)
                ...
                If (Format(CDate(data(y).Fecha_irislab), "dd/MM/yyyy") <> "01/01/1980") Then
                    Mx_Data(4, y) = Format(CDate(data(y).Fecha_irislab), "dd/MM/yyyy HH:mm")
                Else
                    Mx_Data(4, y) = ""
                End If

                Mx_Data(5, y) = data(y).Muestras_recepcionadas
                Mx_Data(6, y) = data(y).Muestras_enviadas
                Mx_Data(7, y) = data(y).Folio_Hoja_trabajo
                If (Format(CDate(data(y).Fecha_envio_HGF), "dd/MM/yyyy") <> "01/01/1980") Then
                    Mx_Data(8, y) = Format(CDate(data(y).Fecha_envio_HGF), "dd/MM/yyyy")
                Else
                    Mx_Data(8, y) = ""
                End If
                If (Format(CDate(data(y).Fecha_recepcion_Resultados), "dd/MM/yyyy") <> "01/01/1980" And data(y).Fecha_recepcion_Resultados <> "1/1/1980 12:00:00 AM") Then
                    Mx_Data(9, y) = Format(CDate(data(y).Fecha_recepcion_Resultados), "dd/MM/yyyy")
                Else
                    Mx_Data(9, y) = ""
                End If
                If (Format(CDate(data(y).Fecha_Validacion_en_Irislab), "dd/MM/yyyy") <> "01/01/1980" And data(y).Fecha_Validacion_en_Irislab <> "1/1/1980 12:00:00 AM") Then
                    Mx_Data(10, y) = Format(CDate(data(y).Fecha_Validacion_en_Irislab), "dd/MM/yyyy")
                Else
                    Mx_Data(10, y) = ""
                End If
        ...
        sl.SetCellValue("E8", "Fecha y hora ingreso Irislab")
        sl.SetCellValue("F8", "Muestras recepcionadas")
        sl.SetCellValue("G8", "Muestras enviadas")
        sl.SetCellValue("H8", "Folio Hoja trabajo")
        sl.SetCellValue("I8", "Fecha envio HGF")
        sl.SetCellValue("J8", "Fecha recepcionada Resultados")
        sl.SetCellValue("K8", "Fecha Validacion en Irislab")

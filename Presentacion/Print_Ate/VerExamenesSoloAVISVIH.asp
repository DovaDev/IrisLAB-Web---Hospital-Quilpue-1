...
<%
	...
	Id_atencion_ = oid_empresa '319 'session("ID_ATEN")
 	...
 	Set rst_BM3 = connBM3.execute("IRISLABWEB_BUSCA_LISTADO_DE_EXAMENES_CODIGO_FONASA3_VIH '"&Id_atencion_&"', '"& ID_PERFIL_NUEVO_REAL &"' ")	
			...
			ELSE
				IF w_IMP_SOLA(A__) = 1 THEN
					...
					'contLINEA_ = contLINEA_ + 2
                        ...
                        CALL Imprime_Linea_Datos_Resultados2(contLINEA_)
						contLINEA_ = contLINEA_ + 1
    ELSE
    CALL Imprime_Linea_Datos_Resultados2(contLINEA_)
					END IF					
					...
					WEND
					CALL IMPRIME_FECHA_HOY2(contLINEA_,w_ID_CF(A__),w_ID_PRUEBA)
...
Function Imprime_Linea_Datos_Resultados(contLINEA_)
    ...
    Function Imprime_Linea_Datos_Resultados2(contLINEA_)
...
'Response.Write("<script>alert('"&DATO_TITU&"');</script>")	
	...
	Params("y") = YY
	Params("size") = 8  
    IF DATO_TITU = "VIH"  or DATO_TITU = "Test VIH" or DATO_TITU = "V.I.H.-4ta- Generacion(Ac.+Ag-P-24" or DATO_TITU = "V.I.H.-4ta- Generacion(Ac.+Ag-P-24)" or DATO_TITU = "Anticuerpos H.I.V 4ª Generacion (Ac. + Ag P-24)"  THEN
			...
			IF Trim(FR_TEXTO_) <> ":" THEN
				IF FR_COLUMNA_ < 4500 THEN
					...
					IF CLng(FR_COLUMNA_) < 551 THEN
						...
						Params("y") = YY
						Params("size") = 6.5
    IF FR_TEXTO_ = "VIH"  or FR_TEXTO_ = "Test VIH" or FR_TEXTO_ = "V.I.H.-4ta- Generacion(Ac.+Ag-P-24" or FR_TEXTO_ = "V.I.H.-4ta- Generacion(Ac.+Ag-P-24)" or FR_TEXTO_ = "Anticuerpos H.I.V 4ª Generacion (Ac. + Ag P-24)"  THEN
    FR_TEXTO_= VNom&VApe&VFnac&VRut
    END IF
...
FUNCTION IMPRIME_FECHA_HOY(contLINEA_,w_ID_CF,w_ID_PRUEBA)
    ...
    FUNCTION IMPRIME_FECHA_HOY2(contLINEA_,w_ID_CF,w_ID_PRUEBA)
	...
	WHILE NOT rst_BM4.eof
		...
		IF FR_OBJETO_ ="Iris_Nombre" THEN
			IF Trim(FR_TEXTO_) <> ":" THEN
				XX = 30
				YY = 665 - (contLINEA_ * 11)				
				Params("x") = ((COLUMNA_ * 648)/8000)
				Params("y") = (630- ((FILA_ * 630)/8500))		
				Params("size") = 6.5
    IF FR_TEXTO_ = "VIH"  or FR_TEXTO_ = "Test VIH" or FR_TEXTO_ = "V.I.H.-4ta- Generacion(Ac.+Ag-P-24" or FR_TEXTO_ = "V.I.H.-4ta- Generacion(Ac.+Ag-P-24)" or FR_TEXTO_ = "Anticuerpos H.I.V 4ª Generacion (Ac. + Ag P-24)" THEN
    FR_TEXTO_= VNom&VApe&VFnac&VRut
    END IF
...
function LLENA_ENCABEZADO(ID_PER)
    ...
    ape_lefttrimeado =  LTrim(rst_BM5("PAC_APELLIDO"))

    ArrApe = Split(ape_lefttrimeado," ")
    ...
    'Page.Canvas.DrawText VNom, Params, Font

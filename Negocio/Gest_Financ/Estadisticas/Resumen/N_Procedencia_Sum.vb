...
Imports System.Web
Public Class N_Procedencia_Sum
    Dim DD_Gen_Activos As D_Gen_Activos
    Dim DD_Prev As D_Procedencia_Sum
    Sub New()
        DD_Gen_Activos = New D_Gen_Activos
        DD_Prev = New D_Procedencia_Sum
    End Sub
    Function IRIS_WEBF_BUSCA_LIS_ADM_RESU_LUGAR(ByVal ID_PRE As Long, ByVal Date_01 As Date, ByVal Date_02 As Date) As List(Of E_IRIS_WEBF_BUSCA_LIS_ADM_RESU_LUGAR)
        Return DD_Prev.IRIS_WEBF_BUSCA_LIS_ADM_RESU_LUGAR(ID_PRE, Date_01, Date_02)
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal PREVE As Long, ByVal DATE1 As String, ByVal DATE2 As String) As String
        ...
        'Realizar Consulta
        Data_Proc = N_Proc.IRIS_WEBF_BUSCA_LIS_ADM_RESU_LUGAR(PREVE, DATE1, DATE2)
        ...
        Next x
        'Llenar Matriz
        For y = 0 To (Data_Proc.Count - 1)
            If (y > 0) Then
                ReDim Preserve Mx_Data(6, y)
            End If
            Mx_Data(0, y) = Data_Proc(y).PROC_DESC
            Mx_Data(1, y) = Data_Proc(y).TOTAL_ATE
            Mx_Data(2, y) = Data_Proc(y).TOT_FONASA
            Mx_Data(3, y) = Data_Proc(y).TOTA_SIS
            Mx_Data(4, y) = Data_Proc(y).TOTA_USU
            Mx_Data(5, y) = Data_Proc(y).TOTA_COPA
            Mx_Data(6, y) = CInt(Data_Proc(y).TOTA_USU + Data_Proc(y).TOTA_COPA)
        Next y
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Resumen de Atenciones por Lugar de Toma de Muestra")
        'titulo de la tabla
        sl.SetCellValue("B2", "Resumen de Atenciones por Lugar de Toma de Muestra")
        If (PREVE = 0) Then
            sl.SetCellValue("B3", "Procedencia: Todas")
        Else
            sl.SetCellValue("B3", "Procedencia: " & Data_Proc(0).PROC_DESC)
        End If
        ...
        Else
            Dim Ruta_save_local As String = System.Web.HttpContext.Current.Server.MapPath("~/")
            Dim Relative_Path As String = "IRISPDFDERIVADOS\" & Data_Proc(0).PROC_DESC & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
            sl.SaveAs(Ruta_save_local & Relative_Path) 'Abrir el Archivo
            'Devolver la url del archivo generado
            Return MAIN_URL & "/" & Replace(Relative_Path, "\", "/")
        End If

...
Public Class Rev_Deter_Exa
    ...
    Public Shared Function Llenar_DataTable(ByVal DESDE As String, ByVal HASTA As String,
        ...
        Data = NN.IRIS_BUSCA_EXAMEN_DET_ESTADISTICA_CHECK_TODOS_2(CDate(DESDE), CDate(HASTA), ID_CF, ID_EST)
    ...
    Public Shared Function FormatValue(value As String, decimalPlaces As Integer) As String
        If String.IsNullOrEmpty(value) OrElse value = "." OrElse value = "-" Then
            Return ""
        End If
        ...
        If Double.TryParse(value, number) Then
            Return CutDecimals(number, decimalPlaces).ToString()
        Else
            Return value
        End If
    End Function

    ' Función para cortar decimales
    Public Shared Function CutDecimals(number As Double, decimals As Integer) As Double
        Dim factor As Double = Math.Pow(10, decimals)
        Return Math.Round(number * factor) / factor
    End Function

    Private Shared Function SanitizeXmlString(input As String) As String
        If String.IsNullOrEmpty(input) Then
            Return String.Empty
        End If

        ' Reemplaza caracteres no válidos con una cadena vacía
        Dim sanitizedString As String = System.Text.RegularExpressions.Regex.Replace(input, "[^\u0020-\u007E]", "")
    ...
    Public Shared Function Excel(ByVal DOMAIN_URL As String, ByVal DESDE As String, ByVal HASTA As String,
        ...
        If (Data.Count > 0) Then
            ...
            For y = 0 To (Data.Count - 1)
                ...
                If (Data(y).PAC_FNAC <> "") Then
                    Mx_Data(6, y) = Data(y).PAC_FNAC
                Else
                    Mx_Data(6, y) = ""
                End If
                ...
                Dim resultado As String = Data(y).TP_RESUL_COD.ToString()
                Mx_Data(14, y) = resultado

                'sl.SetCellValue(14, y, resultado)
                'sl.SetCellStyle(14, y, Style)
                ...
                If (Data(y).ATE_RESULTADO = "") Then
                    ...
                    If (Data(y).ATE_RESULTADO_NUM = "") Then
                        Mx_Data(15, y) = ""
                    Else
                        Mx_Data(15, y) = SanitizeXmlString(Data(y).ATE_RESULTADO_NUM.ToString())
                    End If
                Else
                    Mx_Data(15, y) = SanitizeXmlString(Data(y).ATE_RESULTADO.ToString())
                End If
                ...
                Mx_Data(18, y) = ""
                ' Asignar valores a Mx_Data(19, y)
                If Not String.IsNullOrEmpty(Data(y).ATE_R_DESDE) AndAlso Not IsNothing(Data(y).PRU_DECIMAL) Then
                    Mx_Data(19, y) = FormatValue(Data(y).ATE_R_DESDE, Data(y).PRU_DECIMAL)
                ElseIf Not String.IsNullOrEmpty(Data(y).ATE_RR_DESDE) Then
                    Mx_Data(19, y) = FormatValue(Data(y).ATE_RR_DESDE, Data(y).PRU_DECIMAL)
                Else
                    Mx_Data(19, y) = ""
                End If

                ' Asignar valores a Mx_Data(20, y)
                If Not String.IsNullOrEmpty(Data(y).ATE_R_HASTA) AndAlso Not IsNothing(Data(y).PRU_DECIMAL) Then
                    Mx_Data(20, y) = FormatValue(Data(y).ATE_R_HASTA, Data(y).PRU_DECIMAL)
                ElseIf Not String.IsNullOrEmpty(Data(y).ATE_RR_HASTA) Then
                    Mx_Data(20, y) = FormatValue(Data(y).ATE_RR_HASTA, Data(y).PRU_DECIMAL)
                Else
                    Mx_Data(20, y) = ""
                End If
            Next y
        ...
        For y = 0 To Mx_Data.GetUpperBound(1)
            For x = 0 To Mx_Data.GetUpperBound(0)
                sl.SetCellValue(y + Excel_y, x + 1, Mx_Data(x, y).ToString())
                sl.SetCellStyle(y, style)
            Next x
            ltabla += 1
        Next y
        ltabla += 8

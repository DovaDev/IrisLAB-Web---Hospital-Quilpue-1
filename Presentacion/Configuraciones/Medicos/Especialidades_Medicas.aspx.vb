...
Public Class Especialidades_Medicas
    ...
    End Function
    <Services.WebMethod()>
    Public Shared Function IRIS_WEBF_BUSCA_DOC_ESPECIALIDAD() As String
        'Declaraciones del Serializador
        Dim Serializer As New JavaScriptSerializer
        Dim str_Builder As New StringBuilder
        Dim datas As String = ""
    ...
    End Function
    <Services.WebMethod()>
    Public Shared Function IRIS_WEBF_UPDATE_DOC_ESPECIALIDAD(ByVal ID_ESP As Integer, ByVal ESP_COD As String, ByVal ESP_DES As String, ByVal ID_ESTADO As Integer) As String
        'Declaraciones del Serializador
        Dim Serializer As New JavaScriptSerializer
        Dim str_Builder As New StringBuilder
        Dim datas As String = ""
        ...
        Dim NN As N_IRIS_WEBF_UPDATE_DOC_ESPECIALIDAD = New N_IRIS_WEBF_UPDATE_DOC_ESPECIALIDAD
        numerin = NN.IRIS_WEBF_UPDATE_DOC_ESPECIALIDAD(ID_ESP, ESP_COD, ESP_DES, ID_ESTADO)
        If (numerin > 0) Then
            'Serializar con JSON
            Serializer.MaxJsonLength = 999999999
    ...
    End Function
    <Services.WebMethod()>
    Public Shared Function IRIS_WEBF_GRABA_DOC_ESPECIALIDAD(ByVal ESP_COD As String, ByVal ESP_DES As String, ByVal ID_ESTADO As Integer) As String
        'Declaraciones del Serializador
        Dim Serializer As New JavaScriptSerializer
        Dim str_Builder As New StringBuilder
        Dim datas As String = ""
        'Declaraciones internas
        Dim numerin As Integer = 0
        Dim NN As N_IRIS_WEBF_GRABA_DOC_ESPECIALIDAD = New N_IRIS_WEBF_GRABA_DOC_ESPECIALIDAD
        numerin = NN.IRIS_WEBF_GRABA_DOC_ESPECIALIDAD(ESP_COD, ESP_DES, ID_ESTADO)
    ...
    Public Shared Function Excel(ByVal DOMAIN_URL As String) As String
        ...
        Dim NN As N_IRIS_WEBF_BUSCA_DOC_ESPECIALIDAD = New N_IRIS_WEBF_BUSCA_DOC_ESPECIALIDAD
        data_paciente = NN.IRIS_WEBF_BUSCA_DOC_ESPECIALIDAD()
        Dim titulo As String = "Especialidades Médicas"
        Dim Mx(3, 0) As Object
        For y = 0 To (data_paciente.Count - 1)
            If (y > 0) Then
                ReDim Preserve Mx(3, y)
            End If
            Mx(0, y) = data_paciente(y).ID_ESPECIALIDAD
            Mx(1, y) = data_paciente(y).ESP_COD
            Mx(2, y) = data_paciente(y).ESP_DESC
